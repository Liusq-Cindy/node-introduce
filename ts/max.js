var arr1 = [1, 2, 3, 4, 51, 51, 51, 51, 51, 6, 7, 8, 9, 51, 51, 51, 51, 51, 51, 0, 2]; //定义数组
// 方式一：遍历之后直接找出元素及对应次数
console.time('f1');
...
// 方法三：正则
console.time('f3');
function maxCount3(arr) {
    var newarr = arr.sort(function (a, b) { return a - b; }).toString() + ','; //把数组转换为字符串
    var num = Math.floor(arr.length / 2) - 1; // 因为正则\1会多匹配一个，因此这里 -1
    var reg = "/(\\d+,)\\1{" + num + ",}/g"; // 带变量的正则表达式
    var testEXP = newarr.match(eval(reg));
    var count = testEXP[0].split(',').length - 1;
    var answer = testEXP[0].split(',')[0];
